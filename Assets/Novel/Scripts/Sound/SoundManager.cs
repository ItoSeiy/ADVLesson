...
public class SoundManager : SingletonMonoBehaviour<SoundManager>
{
    ...
    public AudioSource UseSFX(string name)
    {
        ...
        if (!Array.Exists(_soundObjParam.Params, x => x.Name == name))
        {
            Debug.LogError($"{name}というサウンドが見つかりませんでした 誤字又は設定のし忘れがあります。");
            return null;
        }

        var newSound = Instantiate(Array.Find(_soundObjParam.Params, x => x.Name == name).Audio, _audioObjParent.transform);
    ...
    public class SoundPoolParams
    {
        ...
        [SerializeField] 
        private SoundPoolParam[] soundPoolParams;
